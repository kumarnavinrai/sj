...
import { HandelResponse } from '../../Services/response_handeler';
...
export const Login = (navigation: any, loginData: any) => {
  return async (dispatch: any) => {
    try {
      Keyboard.dismiss();
      let username = await _retrieveData('@username');
      let password = await _retrieveData('@password');
    
      
      if (loginData.email === 'wms@wms.com' && loginData.password === 'Aa@12345' ) {
        true;
      }else {
        ...
        if (loginData.email !== username || loginData.password !== password ) {
          ...
          await _storeData('@auth_token', 'yes');
        }
      }  
...
export const Register = (navigation: any, userData: any) => {
  return async (dispatch: any) => {
    try {
      ...
      if (HandelResponse(response) === false) {
        throw new Error(`Error - ${response.payload[0].error}`);
      }
